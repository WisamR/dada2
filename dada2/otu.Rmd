...
Set the path to the fastq files, it is better to be in the same folder with your dada2.R file. The experimental data is from [DADA2 tutorial](https://benjjneb.github.io/dada2/tutorial.html).
```{r path}
...
The critical parameters we chose are the truncation lengths of **240** (forward) and **170** (reverse). 
...
With highly variable amlicons, you will need to use an outside program to remove primers prior to running the dada2 workflow. If you are in that scenario.
...
**We recommend and use [GTDB](https://zenodo.org/record/2541239#.XZtA9yBS-Uk) for 16S data, since it is well updated.**
